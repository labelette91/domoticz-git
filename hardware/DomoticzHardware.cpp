...
void CDomoticzHardwareBase::SendTempHumSensor(const int NodeID, const int BatteryLevel, const float temperature, const int humidity, const std::string &defaultname, const int RssiLevel /* =12 */, int psubtype )
{
	RBUF tsen;
	memset(&tsen, 0, sizeof(RBUF));
...
void CDomoticzHardwareBase::SendSwitchRaw(const int NodeID, const int ChildID, const int BatteryLevel, const bool bOn, const double level, const std::string &defaultname, const int RssiLevel /* =12 */)
{
	//make device ID
	unsigned char ID1 = (unsigned char)((NodeID & 0xFF000000) >> 24);
	unsigned char ID2 = (unsigned char)((NodeID & 0xFF0000) >> 16);
